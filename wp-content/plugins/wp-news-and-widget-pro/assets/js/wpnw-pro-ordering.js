jQuery( function( $ ) {
	...
	$(document).on('click', '.wpnw-save-order', function(){
        ...
        $.post(ajaxurl,data,function(response) {
			...
			if( result.success == 1 ) {
				current_obj.closest('.wrap').find('h1:first').after('<div class="updated notice notice-success is-dismissible wpnw-notice"><p>'+result.msg+'</p></div>');
			} else if( result.success == 0 ){
				current_obj.closest('.wrap').find('h1:first').after('<div class="error notice notice-error is-dismissible wpnw-notice"><p>'+result.msg+'</p></div>');
			}
