...
// Action to add menu
add_action('admin_menu', 'wpbaw_pro_register_designs_page');
 ...
 * @since 1.1.7
 */
function wpbaw_pro_register_designs_page() {
	add_submenu_page( 'edit.php?post_type='.WPBAW_PRO_POST_TYPE, __('Blog Designs', 'wp-blog-and-widgets'), __('Blog Designs', 'wp-blog-and-widgets'), 'manage_options', 'wpbaw-pro-designs-page', 'wpbaw_pro_designs_page' );
}
	...
	<div class="wrap wpbaw-wrap">
		...
		<h2 class="nav-tab-wrapper">
			...
			foreach ($wpbaw_feed_tabs as $tab_key => $tab_val) {
				...
				$tab_link 	= add_query_arg( array('post_type' => WPBAW_PRO_POST_TYPE, 'page' => 'wpbaw-pro-designs-page', 'tab' => $tab_key), admin_url('edit.php') );
			?>
		...
		<?php 
			if( isset($_GET['tab']) && $_GET['tab'] == 'plugins-feed' ) {
				echo wpbaw_pro_get_design( 'plugins-feed' );
			} else {
				echo wpbaw_pro_get_design();
			}
		?>
 ...
 * @since 1.1.7
 */
function pro_blogdesign_admin_style() {

	global $current_screen;

	// If page is our plugin design feed page
	if( isset($current_screen->id) && $current_screen->id == 'blog_post_page_wpbaw-pro-designs-page' ) {
...
add_action('admin_head', 'pro_blogdesign_admin_style');
 ...
 * @since 1.1.7
 */
function wpbaw_pro_get_design( $feed_type = '' ) {
	
	$active_tab = isset($_GET['tab']) ? $_GET['tab'] : 'design-feed';
	
	$cache = get_transient( 'wpbaw_pro_' . $active_tab );
	...
	if ( false === $cache ) {
		...
		if( $feed_type == 'plugins-feed' ) {
			...
			$url = 'http://wponlinesupport.com/plugin-data-api/wp-blog-and-widgets/wp-blog-and-widgets-pro.php';
		}
		...
		if ( ! is_wp_error( $feed ) ) {
			if ( isset( $feed['body'] ) && strlen( $feed['body'] ) > 0 ) {
				...
				set_transient( 'wpbaw_pro_' . $active_tab, $cache, 432000 );
			}
		} else {
