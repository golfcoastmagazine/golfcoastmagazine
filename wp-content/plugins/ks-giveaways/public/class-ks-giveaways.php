...
EOF
        ...
        if (!is_preview() && $_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (KS_Helper::is_running($post)) {
                ...
                if(get_option(KS_GIVEAWAYS_OPTION_GIVEAWAYS_ASK_NAME)) {
                    $calculated_sig = md5($right_answer . '|' . $email . '|' . $first_name);
                } else {
                ...
                } else if (get_option(KS_GIVEAWAYS_OPTION_CAPTCHA_SITE_KEY) && !$this->verify_captcha($_POST['g-recaptcha-response'])) {
                  $GLOBALS['ks_giveaways_error_message'] = 'Google reCaptcha verification failed.';
                } else if ($calculated_sig != $passed_sig) {
                ...
                } else if (get_option(KS_GIVEAWAYS_OPTION_GIVEAWAYS_ASK_NAME) and !$first_name) {
     ...
     */
    public function sync_provider_email_address($email_address, $global = false)
    {
        /*
        ...
        if (KS_Giveaways_Aweber::is_valid()) {
            if ($list_id) {
                ...
                $status['Aweber'] = $cls->add_subscriber($list_id, $email_address);
            } else {
                $status['Aweber'] = 3;
            }
        }
        ...
        if (KS_Giveaways_Mailchimp::is_valid()) {
            if ($list_id) {
                ...
                $status['MailChimp'] = $cls->add_subscriber($list_id, $email_address);
            } else {
                $status['MailChimp'] = 3;
            }
        }
        ...
        if (KS_Giveaways_GetResponse::is_valid()) {
            if ($campaign_id) {
                ...
                $status['GetResponse'] = $cls->add_subscriber($campaign_id, $email_address);
            } else {
        ...
        if (KS_Giveaways_CampaignMonitor::is_valid()) {
            if ($list_id) {
                ...
                $status['Campaign Monitor'] = $cls->add_subscriber($list_id, $email_address);
            } else {
    ...
    public function contestant_confirmed($contestant)
    {
        ...
        $this->sync_provider_email_address($contestant->email_address);
    }
    ...
    public function new_contestant_added($email_address)
    {
        ...
        $this->sync_provider_email_address($email_address);
    }
    ...
    public function add_contestant_entry($contest_id, $email, $referral_id = null, $first_name = null)
    {
        ...
        if (!$existing_contestant) {
            do_action('ks_giveaways_add_contestant', $email);
        }
