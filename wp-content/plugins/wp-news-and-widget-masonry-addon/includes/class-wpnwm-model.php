...
class Wpnwm_Model {
	...
	function wpnwm_get_news( $args = array() ) {

		// Taking defaults
		$result_data = array();

		$wpnwm_args['post_type'] 		= !empty($args['post_type']) 		? $args['post_type'] : WPNWM_POST_TYPE;
		$wpnwm_args['posts_per_page'] 	= !empty($args['posts_per_page']) 	? $args['posts_per_page'] : -1;

		// get order by records
		$wpnwm_args['order']		= !empty($args['order']) 	? $args['order'] 	: 'DESC';
		$wpnwm_args['orderby'] 	= !empty($args['orderby']) 		? $args['orderby'] 	: 'date';

		// Show per page records
		if(isset($args['paged']) && !empty($args['paged'])) {
			$wpnwm_args['paged']	=	$args['paged'];
		}

		// Pagination Parameter
		if( isset($args['offset']) ){
			$wpnwm_args['offset'] = $args['offset'];
		}

		// Meta query
		if(isset($args['meta_query']) && !empty($args['meta_query'])) {
			$wpnwm_args['meta_query'] = $args['meta_query'];
		}

		// Taxonomy query
		if(isset($args['tax_query']) && !empty($args['tax_query'])) {
			$wpnwm_args['tax_query'] = $args['tax_query'];
		}

		// Get only specific post
		if( isset($args['post_in']) && !empty($args['post_in']) ) {
			$wpnwm_args['post__in'] = $args['post_in'];	
		}

		// Exclude some posts
		if( isset($args['post_not_in']) && !empty($args['post_not_in']) ) {
			$wpnwm_args['post__not_in'] = $args['post_not_in'];	
		}

		// Run WP Query
		$result = new WP_Query( $wpnwm_args );
		...
		if(isset($args['getcount']) && $args['getcount'] == '1') {
			...
			$result_data = wpnwm_object_to_array($result->posts);
